...
resource "aws_route" "goad_internet_route" {
  route_table_id         = aws_vpc.goad_vpc.main_route_table_id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.goad_vpc_gateway.id
}

# Gateway which allows outbound and inbound internet access to the VPC
resource "aws_internet_gateway" "goad_vpc_gateway" {
  vpc_id = aws_vpc.goad_vpc.id
}

# Create our first subnet (Defaults to 192.168.56.0/24)
resource "aws_subnet" "goad_vpc_subnet_private" {
  vpc_id            = aws_vpc.goad_vpc.id
  cidr_block        = var.SUBNET1_CIDR
  availability_zone = "${var.REGION}a"

  tags = {
    Name = "GOAD Subnet private"
  }
}

# Set DHCP options for delivering things like DNS servers
resource "aws_vpc_dhcp_options" "goad_dhcp" {
...
resource "aws_vpc_dhcp_options_association" "goad_dhcp_assoc" {
  vpc_id          = aws_vpc.goad_vpc.id
  dhcp_options_id = aws_vpc_dhcp_options.goad_dhcp.id
}
